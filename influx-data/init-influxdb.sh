...
if ( [ ! -z "$INIT_USERS" ] || [ ! -z "$INFLUXDB_DB" ] || [ "$(ls -A /docker-entrypoint-initdb.d 2> /dev/null)" ] ) && [ ! "$(ls -d "$META_DIR" 2>/dev/null)" ]; then
	...
	if [ ! -z "$INFLUXDB_RETENTION_NAME" ] && [ ! -z "$INFLUXDB_RETENTION_DURATION" ] && [ ! -z "$INFLUXDB_RETENTION_REPLICATION" ]; then
		CREATE_RET_POL="CREATE RETENTION POLICY $INFLUXDB_RETENTION_NAME ON $INFLUXDB_DB DURATION $INFLUXDB_RETENTION_DURATION REPLICATION $INFLUXDB_RETENTION_REPLICATION DEFAULT"
		DROP_RET_POL="DROP RETENTION POLICY autogen ON $INFLUXDB_DB"
		CREATE_DB_QUERY="$CREATE_DB_QUERY;$CREATE_RET_POL;$DROP_RET_POL"
	fi
